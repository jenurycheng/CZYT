...
class MyApproveViewController: UIViewController {
    ...
    override func viewWillAppear(_ animated: Bool) {
        ...
        if MyApproveViewController.needUpdate == true {
            self.loadData()
            MyApproveViewController.needUpdate = false
        }
    }
    ...
    func loadData()
    {
        if dataSource.myApproves.count == 0 {
            self.view.showHud()
        }
        dataSource.getMyApprove(true, success: { (result) in
    ...
    func loadMore()
    {
        dataSource.getMyApprove(false, success: { (result) in
            self.tableView.reloadData()
            self.tableView.mj_footer.endRefreshing()
            if result.count == 0
            {
...
extension MyApproveViewController : UITableViewDelegate, UITableViewDataSource
{
    func tableView(_ tableView: UITableView, numberOfRowsInSection section: Int) -> Int {
        return dataSource.myApproves.count
    }
    ...
    func tableView(_ tableView: UITableView, didSelectRowAt indexPath: IndexPath) {
        ...
        detail.id = dataSource.myApproves[indexPath.row].advice_id
    ...
    func tableView(_ tableView: UITableView, cellForRowAt indexPath: IndexPath) -> UITableViewCell {
        ...
        cell.updateApprove(dataSource.myApproves[indexPath.row])
        return cell
    }
}
